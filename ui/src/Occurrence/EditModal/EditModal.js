...
export const EditModal = ({ date, slug, isOpen = true, onClose, onSave }) => {
  ...
  const [selectedHostMemberId, setSelectedHostMemberId] = useState(hostMemberId)
            ...
            <Stack space="md">
              <FormControl isRequired>
                ...
                <DayPicker
                  ...
                  {...{ defaultMonth }}
                />
                <FormControl.HelperText>{t`Select any dates that are not already used.`}</FormControl.HelperText>
              </FormControl>
              <FormControl>
                <FormControl.Label>
                  <Trans>Host</Trans>
                </FormControl.Label>
                <Select isClearable onChange={handleChangeHost} {...{ options, value, isLoading }} />
                <FormControl.HelperText>{t`Select the host for this occurrence.`}</FormControl.HelperText>
              </FormControl>
            </Stack>
          </Center>
        </Modal.Body>
