...
import { Button, Menu, Modal, Pressable, Spinner, Stack, Text, ThreeDotsIcon, useDisclose, useToast } from 'native-base'
...
export const AttendanceMenu = ({
  ...
  const handleSetVisitor = async () => {
    ...
    onUpdate()
    toast.show({
      description: isAttended ? t`The member is now a visitor` : t`The member is removed from the gathering`,
      isClosable: true,
    })
  }
    ...
    <>
      <Menu
        trigger={(triggerProps) => (
          <Pressable accessibilityLabel="More options menu" isDisabled={isSaving} {...triggerProps}>
      ...
      <Modal isOpen={isConfirmVisible} onClose={() => setConfirmVisibility(false)}>
        <Modal.Content maxH="212">
          ...
          <Modal.Body>
            <Stack space="md" textAlign="center">
              <Text>
                <Trans>This member won't appear in the future occurrence.</Trans>
              </Text>
              <Text>
          ...
          <Modal.Footer>
            <Button.Group space="sm">
              <Button onPress={() => setConfirmVisibility(false)} variant="ghost">
                <Trans>Cancel</Trans>
              </Button>
              <Button colorScheme="red" isLoading={isSaving} onPress={handleSetVisitor}>
                <Trans>Yes</Trans>
              </Button>
            </Button.Group>
          </Modal.Footer>
